...
CREATE INDEX idx_content_type_app_label ON content_type(app_label);
CREATE INDEX idx_content_type_model ON content_type(model);
CREATE INDEX idx_content_type_public ON content_type(public);

COMMENT ON TABLE content_type IS 'Типы объектов (аналог Django ContentType)';
COMMENT ON COLUMN content_type.app_label IS 'Метка приложения';
COMMENT ON COLUMN content_type.model IS 'Имя модели';
COMMENT ON COLUMN content_type.public IS 'Публичный тип объекта';
COMMENT ON COLUMN content_type.features IS 'Список поддерживаемых функций';
...
CREATE TRIGGER update_content_type_updated 
...
INSERT INTO content_type (app_label, model, public, features) VALUES

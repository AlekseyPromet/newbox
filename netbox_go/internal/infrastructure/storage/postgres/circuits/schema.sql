...
CREATE TABLE circuits_provider_accounts (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    provider_id UUID NOT NULL REFERENCES circuits_providers(id) ON DELETE RESTRICT,
...
CREATE TABLE circuits_circuits (
    ...
    provider_id UUID NOT NULL REFERENCES circuits_providers(id) ON DELETE RESTRICT,
    provider_account_id UUID REFERENCES circuits_provider_accounts(id) ON DELETE RESTRICT,
    type_id UUID NOT NULL REFERENCES circuits_circuit_types(id) ON DELETE RESTRICT,
    status circuits_circuit_status DEFAULT 'active',
    tenant_id UUID REFERENCES tenancy_tenants(id) ON DELETE RESTRICT,
    install_date DATE,
...
CREATE TABLE circuits_circuit_terminations (
    ...
    _provider_network_id UUID REFERENCES circuits_provider_networks(id) ON DELETE RESTRICT,
...
CREATE TABLE circuits_virtual_circuits (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    cid VARCHAR(100) NOT NULL,
    provider_network_id UUID NOT NULL REFERENCES circuits_provider_networks(id) ON DELETE RESTRICT,
    provider_account_id UUID REFERENCES circuits_provider_accounts(id) ON DELETE RESTRICT,
    type_id UUID NOT NULL REFERENCES circuits_virtual_circuit_types(id) ON DELETE RESTRICT,
    status circuits_circuit_status DEFAULT 'active',
    tenant_id UUID REFERENCES tenancy_tenants(id) ON DELETE RESTRICT,
    description TEXT,
    comments TEXT,

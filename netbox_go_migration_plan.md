...
| Архитектура приложения | Hive | Модульная архитектура, чистая архитектура, DDD |
...
### Архитектурный подход Hive

Проект использует архитектуру **Hive** — эволюцию чистой архитектуры (Clean Architecture) и предметно-ориентированного проектирования (DDD), адаптированную для Go.

**Ключевые принципы Hive:**
...
- Hive модули изолируют ответственность по доменам
...
│   ├── modules/                 # Hive модули по доменам
│   │   ├── dcim/                # DCIM модуль
...
-- Таблица кэша (опционально, если не используется etcd напрямую)
...
-- Таблица блокировок (для распределённых блокировок)
...
// NewCache создаёт клиент etcd с поддержкой кластера
func NewCache(cfg EtcdConfig, logger *zap.Logger) (*Cache, error) {
    client, err := clientv3.New(clientv3.Config{
        Endpoints:   cfg.Endpoints,
        DialTimeout: cfg.DialTimeout,
        Username:    cfg.Username,
        Password:    cfg.Password,
        AutoSyncInterval: 30 * time.Second, // Авто-синхронизация с кластером
    })
    if err != nil {
        return nil, err
    }
...
func (c *Cache) Set(ctx context.Context, key string, value []byte, ttl time.Duration) error {
    ...
    _, err = c.client.Put(ctx, key, string(value), clientv3.WithLease(leaseResp.ID))
    return err
}
...
// AcquireLock приобретает распределённую блокировку через etcd
func (c *Cache) AcquireLock(ctx context.Context, lockName string, ttl time.Duration) (*concurrency.Session, error) {
...
func AuthMiddleware(authService auth.Service) echo.MiddlewareFunc {
    return func(next echo.HandlerFunc) echo.HandlerFunc {
        return func(c echo.Context) error {
            user, err := authService.Authenticate(c)
            if err != nil {
                return err
            }
            c.Set("user", user)
...
#### 3.2 Сервисный слой в Hive архитектуре
...
**Пример реализации в Hive архитектуре:**
...
- Jinja2-like шаблоны (использовать Go templates)
...
#### 12.1 Unit тесты в Hive архитектуре
